...
class ModelBuilder:
    ...
    def __init__(self, sets: TimetablerSets):
        ...
        # FIX: ModelBuilder owns its own warnings — does not write back to _sets
        self.warnings: list[str] = []

    def build(self, disable_c1=False, disable_c2=False,
              disable_c2_force=False, quiet=False) -> "ModelBuilder":
        """Build MILP and return self for chaining."""
        s = self._sets

        # FIX: Reset ALL state, including _Z, so build() is truly repeatable
        self.x = {}
        self._var_keys = []
        self._var_list = []
        self._Z = {}
        self.warnings = []  # Also reset warnings on rebuild
    ...
    def _add_variables(self, available_rt, e_idx, r_idx, quiet):
        s = self._sets
        if not quiet:
            print(
    ...
    def _add_c1_room_conflict(self, quiet) -> int:
        s = self._sets
        c1 = 0
        for r in s.R:
            for t in s.T:
    ...
    def _add_c2_core_class_conflict(self, disable_c2_force, quiet) -> int:
        ...
        free_module_events = {
            m: [e for e in s.E if s.event_module.get(e) == m]
            for m in all_core_modules
        }
        free_module_events = {m: evs for m, evs in free_module_events.items() if evs}
        ...
        for (year, prog), modules in s.core_modules_YD.items():
            for t in s.T:
                ...
                if rhs <= 0:
                    ...
                    self.model.addConstraint(xp.Sum(z_vars_yt) <= rhs)
                    c2 += 1
    ...
    def _add_c3_assignment(self, quiet) -> int:
        s = self._sets
        c3 = 0
        for e in s.E:
            vars_e = [
            ...
            if vars_e:
                ...
                # FIX: Write to self.warnings, not back into the sets object
                self.warnings.append(
                    f"Event {e} has no feasible (Room, Timeslot) — "
                    f"model will be infeasible"
                )
        if not quiet:

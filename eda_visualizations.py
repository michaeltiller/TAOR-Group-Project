...
from utils import PLOTS_DIR, DAY_ORDER, HOUR_ORDER, parse_timeslot, get_fill_ratio_color, calculate_fill_ratio
...
def preprocess_timetable(df):
    ...
    df['Fill_Ratio'] = df.apply(
        lambda row: calculate_fill_ratio(row['Event Size'], row['Room Capacity']), axis=1
    )
    return df
...
def plot_room_utilization(df):
    ...
    bars = ax.barh(room_hours.index[::-1], room_hours.values[::-1],
    ...
    for i, (room, hrs) in enumerate(room_hours.items()):
        ...
        ax.text(hrs + room_hours.max() * 0.01,
                len(room_hours) - 1 - i,
                f'{hrs:.1f}h ({pct:.0f}%)',
    ...
    ax.set_title('Room Utilization — Total Scheduled Hours (08:00–17:00)', fontsize=13, fontweight='bold')
...
def plot_fill_ratio(df):
    ...
    # --- Left: histogram ---
    ax_hist.hist(fill.clip(upper=2.0), bins=40, edgecolor='black', alpha=0.7, color='steelblue')
    ...
    ax_hist.axvline(fill.mean(), color='green', linestyle='--', linewidth=1.5,
    ...
    stats_text = (f"Overfilled (>100%): {overfilled} ({100*overfilled/total:.1f}%)\n"
                  f"Well-used (50–100%): {well_used} ({100*well_used/total:.1f}%)\n"
                  f"Underfilled (<50%): {underfilled} ({100*underfilled/total:.1f}%)")
    ax_hist.text(0.97, 0.97, stats_text, transform=ax_hist.transAxes, fontsize=9,
...
def plot_event_distribution(df):
    ...
    ax.set_title('Event Distribution by Day & Hour (08:00–18:00)', fontsize=13, fontweight='bold')
...
def plot_busiest_rooms(df):
    ...
    bars = ax.barh(event_counts.index[::-1], event_counts.values[::-1],
    ...
    for i, (room, count) in enumerate(event_counts.items()):
        ...
        ax.text(count + event_counts.max() * 0.01,
...
def plot_busiest_modules(df):
    ...
    agg = df.groupby('_label').agg(
        count=('Event ID', 'count'),
        core=('Core', 'first') if 'Core' in df.columns else ('_label', lambda x: False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run timetable EDA from a saved Excel file")
    parser.add_argument(
        "--timetable-path",
        type=str,
        required=True,
